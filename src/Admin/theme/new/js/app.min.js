const hkFetch = window.fetch;
window.fetch = async function (...args) {
    return hkFetch(...args).catch(e => {
...
${e}`);
        ...
        if (!cloned.ok || cloned.status != 200) {
            alert(`Не удалось отправить запрос (${cloned.status} ${cloned.statusText})
...
const initializeDropArea = (e, t) => {
    ...
    //             200 == e.target.status && (e = e.target.responseText, l.value = "/uf/images/" + t + "source/" + e, o.setAttribute("src", "/uf/images/" + t + "source/" + e), u.style = null, c.style.display = "none")
...
}), document.querySelectorAll(".tooltip")), openPointModal = (
    tippy(".none", {
...
window.openPointModal = openPointModal, window.closePointModal = closePointModal, window.openDeleteModal = openDeleteModal, window.closeDeleteModal = closeDeleteModal, window.openInfoModal = openInfoModal, window.closeInfoModal = closeInfoModal
...
(() => {
    const controls = document.querySelectorAll(".form-control__tags");

    controls.forEach((control) => {
        const wrap = control.parentElement;
        const textInput = control.querySelector(".form-control__tags-input");
        const selectedList = control.querySelector(".form-control__tags-list");
        const initialList = control.querySelector(".form-control__tags-popup");
        const hiddenInput = control.querySelector(".form-control__tags-value");
        const helpMessage = control.querySelector(".form-control__tags-help");
        const addNewBlock = control.querySelector('.list1__item--add-new');

        const controlName = hiddenInput.name;
        const isReadonly = control.dataset.readonly;
        const isAjax = control.dataset.ajax;

        const debouced = debounce(getData, 300);

        let selected = [];
        const initial = Array.from(control.querySelectorAll(".list1__item"));

        const createTag = function (tag, val) {
            selected.push(val);

            const currentItems = selectedList.querySelectorAll(
            ...
            // Attributes
            text.innerHTML = tag;
            li.classList.add("form-control__tags-item");
            ...
            // Listeners
            li.addEventListener("click", () => removeTag(li, val));
            ...
            if (currentItems.length) {
                currentItems[currentItems.length - 1].insertAdjacentElement(
                    "afterend",
                    li
                );
            } else {
                selectedList.insertAdjacentElement("afterbegin", li);
            }

            initialList.classList.remove("form-control__tags-popup--active");
        };

        const removeTag = function (tag, val) {
            tag.remove();
        ...
        // Берем значения из input value
        //

        const init = function () {
            // selected = hiddenInput.value.split(",");

            if (hiddenInput.value) {
                for (let i = 0; i < hiddenInput.value.split(",").length; i++) {
                    const val = hiddenInput.value.split(",")[i];
                    const listEl = initialList.querySelector(`.list1__item[data-value='${val}']`);
                    ...
                    if (listEl) {
                        ...
                        // Attributes
                        text.innerHTML = listEl.innerHTML;
                        li.classList.add("form-control__tags-item");
                        li.setAttribute("data-value", val);
                        selected.push(val);
            ...
            // initialList.querySelector(".list1__items").innerHTML = "";

            initialList.querySelectorAll('.list1__item').forEach(el => {
                el.addEventListener('click', (e) => {
                    createTag(e.target.innerText, e.target.dataset.value);
                })
            })
        };

        const isValueInValues = (value) => {
            return Array.from(initial).find(
                (el) => el.innerHTML.toLocaleLowerCase() == value.toLocaleLowerCase()
            );
        };

        const deleteExistingValues = () => {
            let flag = false;
            const array = initialList.querySelectorAll(".list1__item");


            for (let i = 0; i < array.length; i++) {
                const element = array[i];

                if (selected.includes(element.getAttribute('data-value'))) {
                    element.style = "display: none !important";
                }
            }
        }

        const renderAjaxData = (data) => {
            console.log(data);
            ...
            for (const [key, value] of Object.entries(data)) {
                ...
                content += `<li data-value="${value.id}" class="list1__item">${value.name}</li>`
            }


            initialList.querySelector('.list1__items').innerHTML = content;

            initialList.querySelectorAll('.list1__item').forEach(item => {
                item.addEventListener('click', () => {
                    createTag(item.innerText, item.dataset.value);
                })
            })

            if (!isElementVisible(initialList)) {
                initialList.classList.add('form-control__tags-popup--top');
            }
        }

        const events = function () {
            textInput.addEventListener("input", (e) => {
                textInput.size = e.target.value.length + 1;

                initialList.querySelector(".list1__items").innerHTML = "";
                initialList.classList.remove('form-control__tags-popup--top');
                ...
                if (!isAjax) {
                    ...
                    initial.forEach((el) => {
                        ...
                        const li = document.createElement("li");
                        li.classList.add("list1__item");
                        li.innerHTML = text;
                        li.setAttribute("data-value", el.dataset.value);

                        if (!el.dataset.value) {
                            li.classList.add('list1__item--add-new');

                            li.addEventListener('click', () => {
                                createTag(textInput.value, textInput.value);
                                li.style.display = "none";
                            })
                        } else {
                            li.addEventListener('click', () => {
                                createTag(text, el.dataset.value);
                            })
                        }
                    ...
                    // Если хоть 1 элемент найден
                    const isFound =
                        Array.from(initialList.querySelectorAll(".list1__item")).filter(
                            (item) => {
                    ...
                    if (!isFound) {
                        if (isReadonly) {
                            ...
                            control.querySelector('.list1__item--add-new').style.display = 'flex';
                            control.querySelector('.list1__item--add-new').setAttribute('data-value', e.target.value);
                            control.querySelector('.list1__item--add-new').querySelector('span').innerText = `«${e.target.value}»`;
                        }
                    } else {
                        control.querySelector('.list1__item--add-new').style.display = 'none';
                        initialList.classList.add("form-control__tags-popup--active");
                    }
                } else {
                    // For Ajax Search
            ...
            textInput.addEventListener("keydown", (e) => {
                if (isReadonly === 'false') {
                    if (e.key === "Enter" && textInput.value.trim()) {
                        const curr = Array.from(selectedList.querySelectorAll(".form-control__tags-item")).map(el => {
                            return el.innerText;
                        });

                        if (!curr.includes(textInput.value.trim()))
                            createTag(e.target.value, e.target.value);
                    }
                } else {
                    if (
                        e.key === "Enter" &&
                        textInput.value.trim() &&
                        isValueInValues(e.target.value)
                    ) {
                        initial.forEach((el) => {
                            if (
                                el.innerHTML.toLocaleLowerCase() ===
                                e.target.value.toLocaleLowerCase()
                            ) {
                                createTag(el.innerHTML, el.getAttribute("data-value"));
        ...
        function getData() {
            ...
            urlParams.set('action', 'searchTags');
            urlParams.set('name', controlName);
            urlParams.set('text', textInput.value);

            const url = window.location.origin + window.location.pathname + '?' + $.param(Object.fromEntries(urlParams.entries()));
            ...
            $.ajax({
                ...
                success: function (data) {
                    renderAjaxData(JSON.parse(JSON.stringify(data)));
                }
            });
        }
...
const initializeFileInput = function (id) {
    const file = document.getElementById(id);
    const isInitied = file.dataset.initialized;

    if (isInitied) return;


    const input = file.querySelector('.form-control__file-input');
    const img = file.querySelector('.form-control__file-img');
    const title = file.querySelector('.form-control__file-title');
    const size = file.querySelector('.form-control__file-size');
    const inputHidden = file.querySelector("input[type=hidden]");


    if (inputHidden.value && img) {
        img.src = inputHidden.value;
        // file.classList.add('form-control__file--active');

    }


    const progrssbar = file.querySelector('.form-control__file-progressbar');
    const progress = file.querySelector('.form-control__file-progressbar .progressbar__progress');
    const head = file.querySelector(".form-control__file-head");


    const btnReset = file.querySelector('.btn-file-reset');
    const btnUpload = file.querySelector('.btn-file-upload');
    const btnChoose = file.querySelector('.btn-file-choose');
    ...
    const postFile = () => {
        ...
        request.upload.addEventListener('progress', function (e) {
            ...
            if (e.loaded <= fileSize) {
                let precent = Math.round((e.loaded / fileSize) * 100);
        ...
        request.addEventListener('load', (e) => {
            if (e.target.status == 200) {
                ...
                // img.setAttribute('src', "/uf/files/" + data);
            }
        })


        request.open('POST', "?action=upload_file");
    ...
    const postImgFile = () => {
        const formData = new FormData();
        const path = inputHidden.dataset.path;
        ...
        request.addEventListener('load', (e) => {
            if (e.target.status == 200) {
                ...
                inputHidden.value = '/uf/images/' + path + 'source/' + data;
                img.setAttribute('src', '/uf/images/' + path + 'source/' + data);
        ...
        request.open('POST', "?action=upload_img");
        request.timeout = 45000;
        request.send(formData);
    }
    ...
    const inputChangeHandler = (fileAdd) => {
        file.classList.add('form-control__file--active');

        let reader = new FileReader();

        title.innerHTML = fileAdd[0].name;
        size.innerHTML = humanFileSize(fileAdd[0].size);
        ...
        reader.readAsDataURL(fileAdd[0]);
    ...
    initializeDropArea(file, handleDrop);

    btnReset && btnReset.addEventListener('click', () => {
        input.value = "";
        inputHidden.value = "";
        file.classList.remove('form-control__file--active');
        img?.setAttribute('src', "");
        size.style.display = "none";
        title.innerHTML = "Перетащите или загрузите файл";
        progrssbar.style = null;
        progress.style = null;
    ...
    input.addEventListener('change', function (e) {
        if (this.files && this.files[0]) {
            ...
            size.innerHTML = humanFileSize(this.files[0].size);
            size.style.display = "block";
...
window.initializeFileInput = initializeFileInput;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const tableRows = tableBody.querySelectorAll(".table__body-row");

    tableRows.forEach(row => {
        const swithcers = row.querySelectorAll(".form-control__switch[data-autoupdate]");

        swithcers.forEach(swithcer => {
            const input = swithcer.querySelector('.form-control__switch-check');

            input.addEventListener("change", (ev) => {
                ev.preventDefault();
                const parent = swithcer.parentElement.parentElement;
                const dataPk = parent.getAttribute("data-pk").trim().replace(/\D/g, "");
                const dataField = parent.getAttribute("data-field").trim().replace(/\W/g, "");

                $.ajax({
                    type: "POST",
                    url: `?ajax=1&action=boolChange&field=${dataField}&pk=${dataPk}`,
                    success: (data) => {
                        console.log(data);
                    }
                })
...
// list4-dropdown-select
(() => {

        const selects = document.querySelectorAll('.list4-dropdown-select');

        selects.forEach(select => {
                const toggle = select.querySelector('.list4-dropdown-select__input');
                const items = select.querySelectorAll('.list1__item');
                const input = select.querySelector('.form-control__input');
                const list = select.querySelector('.list1__items');


                const isAjax = select.dataset.ajax;
                const action = select.dataset.action;
                const id2 = select.dataset.id;
                const hiddenInput = select.querySelector('.list4-dropdown-select__value');
                const name = input.name;

                function init() {
                    const currItem = select.querySelector(`.list1__item[data-value="${hiddenInput.value}"]`);
                    input.value = currItem.innerText;
                ...
                function renderAjaxData(data) {
                    ...
                    for (const [key, value] of Object.entries(data)) {
                        content += `<li data-value="${value.id}" class="list1__item btn-pagination">${value.name}</li>`;
                    }

                    list.innerHTML = content;

                    list.querySelectorAll('.list1__item').forEach(item => {
                        item.addEventListener('click', () => {
                            input.value = item.innerText;
                            input.parentElement.classList.add('form-control--writing');
                            input.parentElement.classList.remove('form-control--error');
                            input.parentElement.classList.add('form-control--success');
                            hiddenInput.value = item.dataset.value;

                            select.querySelector('.form-control-textfield__placeholder').innerHTML = placeholderText;

                            select.classList.remove('list4-dropdown-select--active');
                        });
                    });
                }
                ...
                async function fetchData() {
                    ...
                    const url = `?action=${action}&text=${value}&name=${name}&id=${id2}`;
                ...
                if (input.value) {
                    input.classList.add('form-control-textfield--has-value');
                    toggle.classList.add('form-control--success');
                }
                ...
                if (isAjax) {
                    input.addEventListener('input', debounce(fetchData, 200));
                    ...
                    input.addEventListener('input', (e) => {
                            ...
                            if (value) {
                                toggle.classList.add('form-control--success');
                                toggle.classList.remove('form-control--error');
                            } else {
                                toggle.classList.remove('form-control--success');
                                toggle.classList.add('form-control--error');
                            }
                ...
                toggle.addEventListener('click', () => {
                    select.classList.add('list4-dropdown-select--active');
                });
            }
        )
...
(() => {
    ...
    if (sidebar) {
        const toggleSidebar = document.querySelector('.sidebar__btn');
        const menus = [...document.querySelectorAll('.menu__item'), ...document.querySelectorAll('.menu__sub')];
        // const parentItems = document.querySelectorAll('.menu__item');

        const currentUserLocation = JSON.parse(localStorage.getItem('currentUserLocation')) || null;

        let isSidebarClosed = false;

        //
        // parentItems.forEach(item => {
        //     const path = item.querySelector('.menu__item-path');
        // })


        menus.forEach((menu, menuIdx) => {
            const menuMainLink = menu.querySelector('.menu__item-link') || menu.querySelector('.menu__sub-link');
            ...
            const menuPath = menu.querySelector('.menu__item-path');

            function getInnerListHeight() {
                let maxHeihgt = menuList.scrollHeight;

                menuList.querySelectorAll('.menu__list-sub .menu__list').forEach(list => {
                    maxHeihgt += list.scrollHeight;
                });
            ...
            menuLinks.forEach((link, linkIdx) => {
                link.addEventListener("click", () => {
                    const levelMenu = link.parentElement.getAttribute("data-menu-level");
            ...
            menuSubs.forEach((menuSub, subIdx) => {
                const menuListItems = menuSub ? menuSub.querySelectorAll('.menu__list-item') : [];

                if (menuListItems.length) {
                    menuListItems.forEach((link, linkIdx) => {
                        const levelMenu = link.parentElement.getAttribute("data-menu-level");

                        if (menu.classList.contains("menu__item") && levelMenu >= 4) {
                            link.addEventListener('click', () => {
                                localStorage.setItem('currentUserLocation', JSON.stringify({
                                    menuIndex: menuIdx,
                                    subIndex: subIdx,
            ...
            if (menuList) {
                ...
                menuMainLink.addEventListener("click", () => {

                    !isSidebarClosed ? menu.classList.toggle('active') : menu.classList.toggle('menu__item--active-icon');

                    if (menu.classList.contains('active')) {
                        menuList.style = `max-height: ${getInnerListHeight()}px`;
                        menuList.setAttribute('data-height', getInnerListHeight());

                        setTimeout(() => {
                            menuList.style = 'max-height: max-content';
                        }, 300)

                    } else {
                        menuList.style = `max-height: ${menuList.scrollHeight}px`;

                        setTimeout(() => {
                            menuList.style = `max-height: 0px`;
                ...
                menus.forEach(menu => {
                    ...
                    if (sidebar.classList.contains('sidebar--closed')) {
                        if (menu.classList.contains('active')) {
                            ...
                            menu.classList.add('menu__item--active-icon');
                        }
                    } else {
                        if (menu.classList.contains('menu__item--active-icon')) {
                            menu.classList.add('active');
                            menu.classList.remove('menu__item--active-icon');

                            if (menuList)
                                menuList.style = `max-height: ${menuList.getAttribute('data-height')}px`;
                        }
                    }
        ...
        if (currentUserLocation) {
            const {menuIndex, subIndex, linkIndex} = currentUserLocation;

            // try {
            if (subIndex !== null) {
                const currentMenuItem = document.querySelectorAll('.menu__item')[menuIndex];
                const currentMenuItemList = currentMenuItem.querySelector('.menu__list');
                const currentMenuItemSub = currentMenuItemList.querySelectorAll('.menu__sub')[subIndex];

                const currentMenuLink = currentMenuItemSub.querySelectorAll('.menu__list-item')[linkIndex];

                currentMenuItemList.style = `max-height: unset`;
                currentMenuItemSub.querySelector('.menu__list').style = `max-height: unset`;

                currentMenuItem.classList.add('active');
                currentMenuItemSub.classList.add('active');
                currentMenuLink.classList.add('menu__list-item--active');
            } else {
                const currentMenuItem = document.querySelectorAll('.menu__item')[menuIndex];
                const currentMenuItemList = currentMenuItem.querySelector('.menu__list');
                ...
                if (linkIndex !== null) {
                    const currentMenuLink = currentMenuItem.querySelectorAll('.menu__list-item')[linkIndex];
                    currentMenuLink.classList.add('menu__list-item--active');
                }

                if (currentMenuItemList)
                    currentMenuItemList.style = `max-height: max-content`;
        ...
        if (window.innerWidth < 1401 && window.innerWidth > 991) {
            ...
            menus.forEach((menu, idx) => {
                ...
                if (menu.classList.contains('active') && menuIndex !== idx) {
                    menu.classList.remove('active');
                    menuList.style = `max-height: 0px`;
                    menu.classList.add('menu__item--active-icon');
                }
...
window.onload = () => {
    (() => {
        ...
        dropdowns.forEach(dropdown => {
            const input = dropdown.querySelector('.form-control__dropdown-input');

            const dropdownTop = dropdown.querySelector('.form-control__dropdown-top');
            const current = dropdown.querySelector('.form-control__dropdown-current');
            const items = dropdown.querySelectorAll('.form-control__dropdown-item');
            const list = dropdown.querySelector('.form-control__dropdown-list');

            const dropdownTopWidht = dropdown.clientWidth;
            const listWidth = list.clientWidth;

            const textField = dropdown.querySelector('.form-control__dropdown-text');
            const isAjax = dropdown.dataset.ajax;
            const action = dropdown.dataset.action;
            const id2 = dropdown.dataset.id;
            const enumAttr = input.dataset.enum;
            const enumValue = input.dataset.enumv;

            if (isAjax) {
                const value = input.value
                const url = `?action=${action}&text=${value}&name=${input.name}&id=${id2}`;

                const currentItem = list.querySelector(`.form-control__dropdown-item[data-value="${value}"]`);
                if (currentItem) {
                    textField.value = currentItem.innerHTML;
                }
            }
            ...
            function renderAjaxData(data) {
                ...
                for (const [key, value] of Object.entries(data)) {
                    content += `<div data-value="${value.id}" class="form-control__dropdown-item">${value.name}</div>`;
                }

                list.innerHTML = content;

                list.querySelectorAll('.form-control__dropdown-item').forEach(item => {
                    item.addEventListener('click', () => {
                        input.value = item.dataset.value;
                        textField.value = item.innerHTML;
                        input.dispatchEvent(new Event('change'));
                ...
                if (list.scrollWidth > dropdownTopWidht) {
                    list.style.width = "max-content";
                    const rects = list.getBoundingClientRect();
                    list.querySelectorAll('.form-control__dropdown-item').forEach(item => item.style = 'white-space: initial');
                    list.style.maxWidth = `${container.scrollWidth - rects.left}px`;
            ...
            async function ajaxHandler() {
                ...
                const url = `?action=${action}&text=${value}&name=${input.name}`;
                dropdown.classList.add('form-control__dropdown--active');
            ...
            if (textField && isAjax) {
                textField.addEventListener('input', debounce(ajaxHandler, 200));
            }
            ...
            current.style.maxWidth = `${vw(dropdown.offsetWidth - 24)}`;

            if (window.matchMedia("(max-width: 1401px)").matches) {
                current.style.maxWidth = `${vwh(dropdown.offsetWidth - 24)}`
            }

            if (window.matchMedia("(max-width: 991px)").matches) {
                current.style.maxWidth = `${vwt(dropdown.offsetWidth - 24)}`
            }

            if (window.matchMedia("(max-width: 576px)").matches) {
                current.style.maxWidth = `${vwm(dropdown.offsetWidth - 24)}`
            }
            ...
            items.forEach(item => {
                item.addEventListener('click', () => {
                    ...
                    current.innerHTML = item.innerHTML;
                    dropdown.classList.remove('form-control__dropdown--list-top');
                    dropdown.classList.remove('form-control__dropdown--active');

                    if (enumAttr) {
                        const url = `?action=changeENUM&field=${input.name}&newstatus=${itemValue}&${enumAttr}=${enumValue}`;
            ...
            dropdownTop.addEventListener('click', () => {
                ...
                dropdown.classList.toggle('form-control__dropdown--active');
                if (list.scrollWidth > dropdownTopWidht) {
                    list.style.width = "max-content";
                    const rects = list.getBoundingClientRect();
                ...
                if (isElementVisible(list)) {
                    ...
                    dropdown.classList.add('form-control__dropdown--list-top');
                }
            })
        })
    ...
    // Submit Top Table
    (() => {
        const form = document.querySelector('.table-top-form');

        if (!form) {
            return;
        }

        const dropdowns = form.querySelectorAll('.form-control__dropdown');
        const inputs = form.querySelectorAll('.form-control__input');
        // const sorts = form.querySelectorAll('.form-control__input-sort');

        dropdowns.forEach(dropdown => {
            const dropdownItems = dropdown.querySelectorAll('.form-control__dropdown-item');
        ...
        inputs.forEach(input => {
            input.addEventListener('keydown', (e) => {
                if (e.key === 'Enter') {
                    ...
                    form.append(inputPage);
                    form.submit();
                    // window.location.href = window.location.origin + window.location.pathname + '?p=0';
                }
            })
        })
    })();
...
(() => {
    const cols = document.querySelectorAll('.table__head-col .form-control__dropdown[data-ajax="true"]');

    cols.forEach(col => {
        const list = col.querySelector('.form-control__dropdown-list');
        const input = col.querySelector('.form-control__dropdown-text');
        const form = document.querySelector('.table-top-form');
        const inputHidden = col.querySelector('.form-control__dropdown-input');
        ...
        list.addEventListener('click', (ev) => {
            if (ev.target.classList.contains('form-control__dropdown-item')) {
                inputHidden.value = ev.target.dataset.value;
                form.submit();
            }
        })
    })
...
const initializeDropdown = function (id) {

    const dropdown = document.getElementById(id);
    const isInited = dropdown.getAttribute('data-initialized');
    ...
    function initList() {
        ...
        if (list.scrollWidth > dropdownTopWidht) {
            ...
            list.style.maxWidth = `${window.innerWidth - rects.left - 20 - 12}px`;
        } else {
            list.style.width = "100%";
        }
    }
    ...
    function renderAjaxData(data) {
        let content = '<div data-value="0" class="form-control__dropdown-item">&mdash;</div>';
    ...
    async function ajaxHandler() {
        const value = textField.value
        const dd2 = document.getElementById(id);
        const url = `?action=${action}&text=${value}&name=${input.name}&id=${dd2.dataset.id}`;
    ...
    list.addEventListener('click', (ev) => {
        if (ev.target.classList.contains('form-control__dropdown-item')) {
            const itemValue = ev.target.getAttribute('data-value');
            input.value = itemValue;
            input.dispatchEvent(new Event('change'));

            current.innerHTML = ev.target.innerHTML;
            dropdown.classList.remove('form-control__dropdown--list-top');
            dropdown.classList.remove('form-control__dropdown--active');
        }
    })
...
document.querySelectorAll('.table__row-quantity .form-control__quantity').forEach(control => {
    const input = control.querySelector('.form-control__quantity-input');
    const initialValue = input.value;
    const id = control.dataset.npp;
    const content = control.dataset.content;
    const action = `?action=change_npp&field=npp&${content}=${id}&`;
    ...
    input.addEventListener('input', (ev) => {
        ...
        // Устанавливаем очищенное значение обратно в текстовое поле
    ...
    input.addEventListener('keydown', (ev) => {
        if (ev.key === 'Enter') {
            if (input.value > initialValue) {
                window.location.href += action + 'val=' + input.value;
...
// File input
(() => {
    const files = document.querySelectorAll('.form-control__file');
    ...
    files.forEach(file => {
        ...
        btnReset && btnReset.addEventListener('click', () => {
            ...
            url.set('action', 'deleteField');
            url.set('field_name', inputHidden.name);
...
(function () {
    ...
    const mask = modal.querySelector('.modal-delete__mask');

    mask.addEventListener('click', () => modal.classList.remove('modal-delete--active'));
})();
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (reset) {
        reset.addEventListener('click', (e) => {
            e.preventDefault();
            location.href = './?action=filterReset';
        });
    }
    ...
    const tableBody = document.querySelector(".table__body");
    const tableSelectAll = tableHeader.querySelector(".table-data-select .form-control__checkbox");
    const tableRows = tableBody.querySelectorAll(".table__body-row");

    // Controls Btns
    const controls = document.querySelectorAll('.action-with-select');
    // const btnAdd = document.getElementById("btn-control-add");
    // const btnEdit = document.getElementById("btn-control-edit");
    // const btnCopy = document.getElementById("btn-control-copy");
    // const btnDelete = document.getElementById("btn-control-delete");


    // Table Select All Listener
    tableSelectAll.addEventListener("change", (e) => {
        if (e.target.checked) {
            tableRows.forEach(row => {
                const rowCheckbox = row.querySelector(".table-data-select .form-control__checkbox");
    ...
    const isAnyRowSelected = () => {
        ...
        for (let i = 0; i < tableRows.length; i++) {
            const rowCheckbox = tableRows[i].querySelector(".table-data-select .form-control__checkbox");
